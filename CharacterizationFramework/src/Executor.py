...
#JUNO RELATED STUFF
JUNO_DEBUG_PORT="COM9" #TODO add as json input.. fix magic variable
measureInterrupts=False
measureIntegrity=True
interruptFileName="deletethisganton12"
...
class SSHhandler: #class that handles ssh commands to the target machine
    ...
    #ganton12
    def executeCommandNonBlocking(self,command):
        try:      
            print("damee1")          
            ssh = SSHClient()
            print("damee2")
...
class WorkloadHandler:
    ...
    #TRUE=1
    #FALSE=0
    #ganton12 alaksato apo 10 se 5 PROSOXI na epistrepsi se 10 !!!!!!!!!!
    CHK_ALIVE_FREQ=5
    BIT_FLIP=3
    ...
    UNRESPONSIVE_TIMEOUT=5 ##used to be 5,
    ...
    def __init__(self,sshHandler,serial,run,helperWorkloadScript,workloadStatusOutput,measurePowerScript=None,integrityCheckScript=None,userVoltage=1000):
        self.run=run  
        self.mode=None  
        ...
        self.integrityCheckScript = integrityCheckScript
        self.userVoltage= userVoltage
    ...
    def startWorkload(self):
        ...
        print("empikaStartWorkload")
        WorkloadHandler.processIDtable=[]
        ...
        if (WorkloadHandler.submitFromFramework == True):
            for workload in self.run.workloads:
                #command=str(workload.toHelperScript())+ " "
                command=(str(workload.toHelperScript()));
                tokens=command.split();
                executablename=tokens[1].strip('\"');
                core=tokens[4]
                #"taskset -c " + core + " " +
                command="taskset -c " + core + " " + executablename + " &";
                print(str(command))
                code=self.serial.sendCMD(command,waitTime=WorkloadHandler.UNRESPONSIVE_TIMEOUT);
                ser_out=self.serial.read()
                #print(core);
                pId=ser_out.split('\n')[1].split()[1];
                #print(pId);
                WorkloadHandler.processIDtable.append(pId);
                
            #print(WorkloadHandler.processIDtable);
            #exit();
        else:
            print(str(command))
        ...
        #ganton12 test whether the framework only submits once
        #vazo static counter kai auksanato kathe fora p kanei submit ena experiment
        #print in file
        #sourceFile = open('experimentsID.txt', 'a')
        #print(WorkloadHandler.experimentID, file = sourceFile)
        #sourceFile.close()
        #WorkloadHandler.experimentID+=1;
        
        #increase experimentID
    ...
    def waitWorkloadToFinish(self):
        ...
        while True :
            ...
            #ganton12 I need better precision for initial crashes
            #if (freqCount==30):
            #    WorkloadHandler.CHK_ALIVE_FREQ=5
               
            #print(freqCount)
            #print(WorkloadHandler.CHK_ALIVE_FREQ)
            ...
            if (WorkloadHandler.submitFromFramework == True):
                command="";
                for pId in WorkloadHandler.processIDtable:
                    command=command+"kill -0 "+ pId + " &>/dev/null; echo \"toKill $?\"; "
                flag_check_alive="False"    
                code=self.serial.sendCMD(command, waitTime=WorkloadHandler.UNRESPONSIVE_TIMEOUT)
                ser_out=self.serial.read()
                if(ser_out!=None):
                    exitCode=0
                            ...
                            for i in range(len(tmp)):
                                if(str(tmp[i]).strip()=="toKill"):
                                    if(str(tmp[i+1]).strip()=="$?"):
                                        ...
                                        try:
                                            exitCode=int(tmp[i+1]);
                                            flag_check_alive="True"
                                        except(ValueError):
                                            print("Value Error")
                                            self.systemCrash=True
                                            return WorkloadHandler.SYSTEM_CRASHED;
                                if (int(exitCode)==1):
                                    ...
                                    self.systemCrash=True
                                    return WorkloadHandler.SYSTEM_CRASHED;                                            
                if ser_out!=None and flag_check_alive=="False":
                    print("check alive flag false")
                    self.systemCrash=True
                    return WorkloadHandler.SYSTEM_CRASHED;
                ####
                #self.sshHandler.executeCommandNonBlocking("")          
                #if self.sshHandler.commandStatus == self.sshHandler.COMMAND_FAILED:
                #    self.systemCrash=True
                #    return WorkloadHandler.SYSTEM_CRASHED;
                #print("Passed SSH check")
            else:        
            ...
            if(exitCode==1):##succsfully finished check if output is not corrupt
                    ...
                    for line in lines:
                        if "WORKLOAD_SCRIPT" in line:
                            ...
                            if(len(line.split(" "))<WorkloadHandler.MIN_WORKLOAD_OUTPUT_COLUMNS):
                                ...
                                print("empika sto workliad script aborded proto")
                ...
                if(count_of_lines!=len(self.run.workloads)):
                    self.abnormal_workload_status=True
                    print("empika sto workliad script aborded")
                    return WorkloadHandler.WORKLOAD_SCRIPT_ABORTED #so the script produced abnormal output ?? consider system not responsive??
                ...
                print("return workload script finished succesfully")
...
class Executor:
    ...
    def __init__(self,helperScriptSetup,helperScriptWorkload,workloadStatusOutput,targetHostname="xg2-1.in.cs.ucy.ac.cy",targetSSHusername="zhadji01",targetSSHpassword="agoodpasswd",serialPort="COM3",sysLogParsingScript="/home/zhadji01/printErrorsWithinTimestamps",measurePowerScipt=None,integrityCheckScript=None,userVoltage=1000):
        #self.run=run
        self.targetHostName=targetHostname
        self.targetSSHusername=targetSSHusername
        self.targetSSHpassword=targetSSHpassword
    ...
    #asioko01
        ...
        runExec=Executor(helperScriptSetup,helperScriptWorkload,workloadStatusOutput,targetHostname=targetHostName,targetSSHusername=targetSSHusername,targetSSHpassword=targetSSHpassword,serialPort=serialPort,sysLogParsingScript=sysLogParsingScript,measurePowerScipt=measurePowerScript,integrityCheckScript=integrityCheckScript,userVoltage=userVoltage)
        return runExec
    ...
    def configureJuno(self,run):
        ...
        sshHandler.executeCommand(command="echo performance > /sys/devices/system/cpu/cpu1/cpufreq/scaling_governor")
        ...
        #ganton12 24 multiplier for A72 run.core_to_frequencies 1 instead of 4 
        setFrequency(run.core_to_frequencies[1], portCOM=JUNO_DEBUG_PORT, multiplier=24, supply=7)
    ...
    def configureJunoA53(self,run):
        ...
        if(JUNO_BOOST_INTERRUPTS):
            executeCommand(hostname="10.16.20.168",username="root",password="root",command="pkill selfLoop",background=False)     
        executeCommand(hostname="10.16.20.171",username="root",password="root",command="mount /dev/sdc1 /media/oldDisk1/",background=False)
        if(JUNO_BOOST_INTERRUPTS):
            executeCommand(hostname="10.16.20.168",username="root",password="root",command="pkill selfLoop",background=False)     
            '''
            ...
            executeCommand(hostname="10.16.20.171",username="root",password="root",command=commandInterrupt,background=False)            
            CURRENTCORERUNNING+=1      
            '''           
        setVoltage(2,float(nominal)/1000,JUNO_DEBUG_PORT)
        #setFrequency(frequency,SCRIPTS_BASE+SET_FREQUENCY_PY,SCRIPTS_BASE+SET_FREQUENCY_BAT)
        
        #ganton12 alaksa to multiplier pou 16 se 19 giati to perf evgaze 950MHz otan o multiplier itan 19
        setFrequency(run.core_to_frequencies[0], portCOM=JUNO_DEBUG_PORT, multiplier=19, supply=6)
        ...
        if int(platform) == 4: #4 is xgene3
            while True:
                try:                
                    ...
                    stdin,stdout,stderr=ssh.exec_command(command="ipmitool -H 10.16.20.179 -U ADMIN -P ADMIN -I lanplus power off; sleep 10; ipmitool -H 10.16.20.179 -U ADMIN -P ADMIN -I lanplus power on")
                    self.out=stdout.readlines() #just do this to make exec blocking call
                    self.err=stderr.readlines()       
                    ssh.close()
                    break
    ...
    def execute_full_exp(self,run,CURRENTCORERUNNING,testUntilUnsafe=True):
        ...
        print("next voltage to set " + str(voltage))
        while voltage!=NextVoltageGenerator.NO_MORE_VOLTAGES: 
            ...
            workloadHandler = WorkloadHandler (sshHandler,self.serial,run,self.helperScriptWorkload,self.workloadStatusOutput,self.measurePowerScript,self.integrityCheckScript,self.userVoltage)
            if "juno_a53" in str(run.platform):
                
                #executeCommand(hostname="10.16.20.177",username="root",password="root",command="./getInterruptsA53Plus.sh >> IDLE3CORES0",background=False)
                if(JUNO_BOOST_INTERRUPTS):
                    executeCommand(hostname="10.16.20.168",username="root",password="root",command="pkill selfLoop",background=False)     
                    '''
                    if(CURRENTCORERUNNING==0):
                    ...
                    executeCommand(hostname="10.16.20.168",username="root",password="root",command=commandInterrupt,background=False)       
                    #CURRENTCORERUNNING+=1
            ...
            if measureIntegrity:
                ...
                code = self.serial.sendCMD(integrity_check_command, waitTime=WorkloadHandler.UNRESPONSIVE_TIMEOUT);
                ser_out = self.serial.read()
                pId = ser_out.split('\n')[1].split()[1]
            ...
            print("before startworkload")
            workloadHandler.startWorkload()
            print("after startworkload")
            if(measureInterrupts):
                executeCommand(hostname="10.16.20.168",username="root",password="root",command="/home/root/getInterruptsA53A72Plus.sh >> "+interruptFileName,background=False)
            ...
            #ganton12  
            #crashCore=""
            #if(code==WorkloadHandler.SYSTEM_CRASHED):
            #    if (WorkloadHandler.submitFromFramework == True):
            #        for pId in WorkloadHandler.processIDtable:
            #            code=self.serial.sendCMD("kill -0 "+ pId +" &>/dev/null; echo \"toKill $?\"", waitTime=WorkloadHandler.UNRESPONSIVE_TIMEOUT)
            #            ser_out=self.serial.read()
            #            if(ser_out==None):
            #                break;
            #            exitCode=0
            ...
            #                                    break;                                      
                        #print(ser_out.split('\n')[1]);
            #            if (int(exitCode)==1):
            #                crashCore=crashCore+str(WorkloadHandler.processIDtable.index(str(pId))) + " ";
            #        sourceFile = open('crashCore.txt', 'a')
            #        print(crashCore, file = sourceFile)
            #        sourceFile.close()
            #    code=1
            ...
            if (code==WorkloadHandler.WORKLOAD_SCRIPT_FINISHED_SUCCESFULLY): ##SYSTEM CRASH NOT HAPPENED
                ...
                for workload in run.workloads:
                    ...
                    if(code == workloadHandler.BIT_FLIP):
                        jsonOutput.workloads[i].bitFlip=True
                    else:
                        jsonOutput.workloads[i].bitFlip=None
                    i=i+1   
    ...
    def __handleRestart_(self,platform):
        ...
        if "juno" in str(platform): #TODO fix this in the future
            ...
            ##serial reboot code
            #executeCommand(hostname="10.16.20.171",username="root",password="root",command="umount /media/oldDisk1/",background=False)
            sendRebootCommand(JUNO_DEBUG_PORT)
            start = time.time() 
            while True:
                time.sleep(1)
